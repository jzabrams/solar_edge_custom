...
class SolarEdge:
    ...
    def __init__(
        ...
        timeout: int = 10,
        base_url: yarl.URL | str = _BASE_URL,
    ) -> None:
        ...
        self._base_url = yarl.URL(str(base_url))

        _LOGGER.warning("Using custom SolarEdge client")
    ...
    def _get_site_url(self, site_id: int | str) -> yarl.URL:
        """Get the site URL."""
        return self._base_url.joinpath("site", str(site_id))

    async def get_details(self, site_id: int | str | None = None) -> dict[str, Any]:
        """
        Get details of the SolarEdge system via the /sites/list endpoint.
        If site_id is provided, return that site’s details; otherwise return the first site.
    
        :param site_id: The site ID (optional).
        :return: The details of the SolarEdge system.

        """
        _LOGGER.debug("Fetching site list via custom SolarEdge client")
        data = await self._get_json(self._base_url.joinpath("sites", "list"))
